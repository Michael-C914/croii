...
export default NextAuth({
  ...
  callbacks: {
     ...
     * @param {object} user Información devuelta por la opción de autenticación (Azure o Credentials)
     * @param {object} account Información de la cuenta para la autenticación (Azure o Credentials)
     * La función identifica la opción de Autenticación elegida por el usuario
     * Azure: Recibe el token de autenticación provisto por Azure y lo envia al sistema backend para recibir un token valido dentro del sistema y lo incluye en el token del lado del cliente.
    ...
    async session(session, token) {
      ...
      session.id = dec.payload.user_id;
      session.email = dec.payload.email;
      session.first_name = dec.payload.first_name;
      session.last_name = dec.payload.last_name;
